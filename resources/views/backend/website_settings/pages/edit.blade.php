	...
	<div class="card">
		...
		<form class="p-4" action="{{ route('custom-pages.update', $page->id) }}" method="POST" enctype="multipart/form-data">
			...
			<div class="card-body px-0">
				...
				<div class="form-group row">
					<label class="col-sm-2 col-form-label" for="signinSrEmail">{{translate('Banner')}} <small>({{ translate('Dimensions: 2000 x 1536, Safe Zone 991 x 650') }})</small></label>
					<div class="col-sm-10">
						<div class="input-group" data-toggle="skuploader" data-type="image">
							...
							<input type="hidden" name="banner" class="selected-files" value="{{ $page->banner }}">
						</div>
				...
				<div class="form-group row">
					<label class="col-sm-2 col-from-label">{{translate('Banner Description')}}</label>
					<div class="col-sm-10">
						<textarea class="form-control" name="banner_desc" placeholder="{{translate('Banner Description')}}">{{ $page->getTranslation('banner_desc',$lang) }}</textarea>
					</div>
				</div>
                @php
                    $page_array = array('7','8', '9', '12');
                @endphp
