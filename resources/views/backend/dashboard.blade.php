...
@if(Auth::user()->user_type == 'admin' || in_array('2_2', json_decode(Auth::user()->staff->role->permissions)))
    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="sk-carousel gutters-10 half-outside-arrow" data-items="6" data-xl-items="5" data-lg-items="4" data-md-items="3" data-sm-items="2" data-arrows='true'>
                @foreach (filter_products(\App\Product::where('published', 1)->orderBy('num_of_sale', 'desc'))->limit(12)->get() as $key => $product)
                    <div class="carousel-box">
                        <div class="sk-card-box border border-light rounded shadow-sm hov-shadow-md mb-2 has-transition bg-white">
...
@if(Auth::user()->user_type != 'admin' && !in_array('1', json_decode(Auth::user()->staff->role->permissions)) && !in_array('2_2', json_decode(Auth::user()->staff->role->permissions)))
    <div class="alert alert-primary text-center">
        {{translate('You are logged in to the admin dashboard')}}
    </div>
@endif
