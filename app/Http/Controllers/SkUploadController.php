...
class SkUploadController extends Controller
{
    ...
    public function show_uploader(Request $request){
        return view('uploader.sk-uploader');
    }
    public function upload(Request $request){
        ...
        if($request->hasFile('sk_file')){
            $upload = new Upload;
            $extension = strtolower($request->file('sk_file')->getClientOriginalExtension());

            if(isset($type[$extension])){
                $upload->file_original_name = null;
                $arr = explode('.', $request->file('sk_file')->getClientOriginalName());
                ...
                $path = $request->file('sk_file')->store('uploads/all', 'local');
                $size = $request->file('sk_file')->getSize();

                if($type[$extension] == 'image' && get_setting('disable_image_optimization') != 1){
                    try {
                        $img = Image::make($request->file('sk_file')->getRealPath())->encode();
                        $height = $img->height();
                        $width = $img->width();
